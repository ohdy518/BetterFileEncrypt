...
def fopen(_=any):
    files = tkfd.askopenfilenames(parent=root, title="Select files to Decrypt", filetypes=[("All Files", "*.*")])
    for i in files:
        filelist.append(i)
    deslbl.config(text=f"{str(len(filelist))} file(s) selected: {time.time()}")
def spin(_=any):
    deslbl.config(text=f"New window will open: {time.time()}")
    os.system("python .\\getpin.py")
def enc(_=any):
    ...
    eta = 0
    frun = True
    deslbl.config(text=f"Decrypting: {time.time()}")
    pwf = open("pin.data", "r")
    pw = pwf.read()
    pwf.close()
    pwf = open("pin.data", "w")
    ...
    deslbl.config(text=f"Decrypted: {time.time()}")
...
encbtn = tk.Button(root, text="Decrypt", font=("Consolas", 12), command=enc)
encbtn.pack()
